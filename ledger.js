...
export function addTransaction(chat_id, data) {
  const { user, account, category, note, amount, type, tags } = data

  db.prepare(`
    INSERT INTO transactions
    (chat_id, user, account, category, note, amount, type, tags)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(chat_id, user, account, category, note, amount, type, tags || "")
}
